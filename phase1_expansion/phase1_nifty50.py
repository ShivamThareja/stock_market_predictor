...
SECTOR_MAP = {
    ...
    "Auto": {
        "TMPV.NS": "Tata Motors",  # renamed from TATAMOTORS.NS after the 2025 CV/PV demerger
...
# A ticker can survive the concat as an all-NaN column if its whole batch
# download failed (e.g. delisted/renamed symbol) — don't count those as loaded.
loaded_mask = close_prices.notna().any(axis=0)
tickers_loaded = int(loaded_mask.sum())
print(f"Tickers successfully loaded: {tickers_loaded} / {len(ALL_TICKERS)}")
failed_tickers = list(close_prices.columns[~loaded_mask])
if failed_tickers:
    print(f"  Failed (no data returned): {failed_tickers}")
    close_prices = close_prices.loc[:, loaded_mask]
